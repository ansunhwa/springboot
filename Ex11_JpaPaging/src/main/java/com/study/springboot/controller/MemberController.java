...
public class MemberController {
									...
									Model model)
	{
		 ...
		 *  
		 *  > 생성자
		 *  PageRequest(int page, int size) 기본
		 *  PageRequest(int page, int size, Sort sort)
		 *  PageRequest(int page, int size, Sort.Direction direction, String...)
		 */
		
		int nPage = Integer.parseInt(page) -1;   //0부터 시작하기 때문에
		...
		*/
		...
		Sort sort = Sort.by(Sort.Order.desc("name"));  // 이름의 내림차순
		Pageable pageable = PageRequest.of(nPage, 10, sort);  //해서 10페이지씩 가져옴
		
		//Pageable pageable = PageRequest.of(nPage, 10, Sort.by("name").descending());  //내림차순   ascending() - 오름차순
		
		Page<Member> result = memberService.selectNameLike(name2, pageable);
		
		List<Member> member = result.getContent();        // 실제 객체가 담긴 List<Member>**
		long totalElements = result.getTotalElements();	  // 전체 레코드 수 (23개)
		int totalPages = result.getTotalPages();          // 전체 페이지 수 (3page)
		int size = result.getSize();	                  // 한페이지당 최대 레코드 수
		int pageNumber = result.getNumber() + 1;          // 현재 페이지(0부터 시작하므로 +1) 
